...
func (d *DLL) Proc(name string) *dll类.Proc {
	p, e := d.I查找命令(name)
	if e != nil {
		d.t.Fatal(e)
	}
...
func TestStdCall(t *testing.T) {
	...
	a, _, _ := GetDLL(t, "user32.dll").Proc("UnionRect").I调用(
...
func Test64BitReturnStdCall(t *testing.T) {
	...
	m1, m2, _ = VerSetConditionMask.I调用(m1, m2, VER_MAJORVERSION, VER_GREATER_EQUAL)
	m1, m2, _ = VerSetConditionMask.I调用(m1, m2, VER_MINORVERSION, VER_GREATER_EQUAL)
	m1, m2, _ = VerSetConditionMask.I调用(m1, m2, VER_SERVICEPACKMAJOR, VER_GREATER_EQUAL)
	m1, m2, _ = VerSetConditionMask.I调用(m1, m2, VER_SERVICEPACKMINOR, VER_GREATER_EQUAL)
	...
	r, _, e2 := d.Proc("VerifyVersionInfoW").I调用(
...
func TestCDecl(t *testing.T) {
	...
	a, _, _ := GetDLL(t, "user32.dll").Proc("wsprintfA").I调用(
...
func TestEnumWindows(t *testing.T) {
	...
	counter := 0
	cb := dll类.NewCallback(func(hwnd dll类.Handle, lparam uintptr) uintptr {
		if lparam != 888 {
			t.Error("lparam was not passed to callback")
		}
		b, _, _ := isWindows.I调用(uintptr(hwnd))
		if b == 0 {
	...
	a, _, _ := d.Proc("EnumWindows").I调用(cb, 888)
	if a == 0 {
...
func nestedCall(t *testing.T, f func()) {
	c := dll类.NewCallback(callback)
	d := GetDLL(t, "kernel32.dll")
	defer d.I卸载dll()
	const LOCALE_NAME_USER_DEFAULT = 0
	d.Proc("EnumTimeFormatsEx").I调用(c, LOCALE_NAME_USER_DEFAULT, 0, uintptr(*(*unsafe.Pointer)(unsafe.Pointer(&f))))
}
...
func (f cbFunc) testOne(t *testing.T, dll *dll类.DLL, cdecl bool, cb uintptr) {
	r1, _, _ := dll.I查找命令P(f.cName(cdecl)).I调用(cb)
...
func TestRegisterClass(t *testing.T) {
	...
	mh, _, _ := kernel32.Proc("GetModuleHandleW").I调用(0)
	cb := dll类.NewCallback(func(hwnd dll类.Handle, msg uint32, wparam, lparam uintptr) (rc uintptr) {
	...
	type Wndclassex struct {
		...
		ClsExtra   int32
		WndExtra   int32
		Instance   dll类.Handle
		Icon       dll类.Handle
		Cursor     dll类.Handle
		Background dll类.Handle
		MenuName   *uint16
		ClassName  *uint16
		IconSm     dll类.Handle
	}
	name := dll类.StringToUTF16Ptr("test_window")
	wc := Wndclassex{
		WndProc:   cb,
		Instance:  dll类.Handle(mh),
		ClassName: name,
	}
	wc.Size = uint32(unsafe.Sizeof(wc))
	a, _, err := user32.Proc("RegisterClassExW").I调用(uintptr(unsafe.Pointer(&wc)))
	if a == 0 {
		t.Fatalf("RegisterClassEx failed: %v", err)
	}
	r, _, err := user32.Proc("UnregisterClassW").I调用(uintptr(unsafe.Pointer(name)), 0)
	if r == 0 {
...
func TestOutputDebugString(t *testing.T) {
	d := GetDLL(t, "kernel32.dll")
	p := dll类.StringToUTF16Ptr("testing OutputDebugString")
	d.Proc("OutputDebugStringW").I调用(uintptr(unsafe.Pointer(p)))
}
...
`
	...
	if err != nil {
		t.Fatalf("failed to build dll类: %v - %v", err, string(out))
	}
	dllpath := filepath.Join(tmpdir, outname)

	dll := dll类.I创建P(dllpath)
	defer dll.I卸载dll()

	proc := dll.I查找命令P("cfunc")

	cb := dll类.NewCallback(func(n uintptr) uintptr {
		forceStackCopy()
		return n
	})
...
func TestSyscallN(t *testing.T) {
	...
	for arglen := 0; arglen <= 42; arglen++ {
		...
		int cfunc(%s) { return %s; }`, strings.Join(args, ", "), strings.Join(rets, " && "))
			...
			out, err := cmd.CombinedOutput()
			if err != nil {
				t.Fatalf("failed to build dll类: %v\n%s", err, out)
			}
			dllpath := filepath.Join(tmpdir, outname)
			...
			// proc.I调用() will call I调用命令() internally.
			r, _, err := proc.I调用(params...)
			if r != 1 {
...
func TestBigStackCallbackSyscall(t *testing.T) {
	...
	var ok bool
	proc := dll.I查找命令P("bigStack")
	cb := dll类.NewCallback(func() uintptr {
...
var (
	modwinmm    = dll类.I创建_延迟加载("winmm.dll")
	modkernel32 = dll类.I创建_延迟加载("kernel32.dll")

	procCreateEvent = modkernel32.I创建命令对象("CreateEventW")
	procSetEvent    = modkernel32.I创建命令对象("SetEvent")
)

func createEvent() (dll类.Handle, error) {
	r0, _, e0 := dll类.I调用命令6(procCreateEvent.I取命令地址(), 4, 0, 0, 0, 0, 0, 0)
	if r0 == 0 {
		return 0, syscall.Errno(e0)
	}
	return dll类.Handle(r0), nil
}

func setEvent(h dll类.Handle) error {
	r0, _, e0 := dll类.I调用命令3(procSetEvent.I取命令地址(), 1, uintptr(h), 0, 0)
	if r0 == 0 {
